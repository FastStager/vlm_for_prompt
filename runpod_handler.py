...
def load_essentials():
    ...
    This function is called once on cold start.
    """
...
def handler(job):
    ...
    if not image_url:
        return {"error": "Image URL is a required field."}

    try:
        validate_image_url(image_url)
    except ValueError as e:
        return {"error": f"Image Validation Error: {e}"}

    furniture_config = config.get("FURNITURE_CONFIG", {})
    style_materials = config.get("STYLE_MATERIALS", {})

    _, analysis_messages = create_analysis_prompt(image_url)
    ...
    _, placement_messages = create_placement_prompt(room_type, style, image_url, room_analysis, furniture_config, style_materials)
